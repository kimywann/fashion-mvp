...
사용자가 상품을 탐색하고, 장바구니/주문/결제까지 이어지는 커머스 핵심 플로우를 구현한 MVP입니다.
...
- 로그인 여부에 따라 로컬(Redux)·서버(Supabase) 장바구니를 자동으로 스위칭, 수량 조정·삭제 시 실시간 합계 업데이트
...
- 관리자 페이지에서 상품 목록/메타데이터를 Table UI로 조회.
...
- 로그인/비로그인 장바구니를 하나의 Hook으로 캡슐화해 동일한 UI/UX로 유지
- TanStack Query Mutation + Optimistic Update를 활용한 장바구니 수량 조정/삭제/추가
